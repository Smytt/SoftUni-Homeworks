...
namespace _06.Catch_the_Thief
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if ( thiefID < 0)
            {
                sentence = (long)Math.Ceiling((decimal)thiefID / SByte.MinValue);
            }
            else
            {
                sentence = (long)Math.Ceiling((decimal)thiefID / SByte.MaxValue);
            }

            string plural = "";
            if (sentence != 1) plural = "s";

            Console.WriteLine($"Prisoner with id {thiefID} is sentenced to {sentence} year{plural}");
        }
    }
}
